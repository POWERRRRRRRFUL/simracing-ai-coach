...
- **Reasoning model support** — automatic compatibility for thinking/reasoning models (DeepSeek-Reasoner, etc.)
...
│   ├── llm/               Provider-agnostic LLM facade, response normalization, prompt templates
│   │   └── providers/     Abstract base + OpenAI-compatible transport (httpx)
...
- Provider abstraction — `BaseProvider` / `OpenAICompatibleProvider` separate HTTP transport from response normalization; reasoning models (DeepSeek-Reasoner, etc.) are handled automatically
...
- **推理模型支持** — 自动适配思考/推理类模型（DeepSeek-Reasoner 等）
...
- 提供商抽象 — `BaseProvider` / `OpenAICompatibleProvider` 将 HTTP 传输与响应归一化分离；推理模型（DeepSeek-Reasoner 等）自动适配
