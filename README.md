...
- **Provider-agnostic** — works with OpenAI, DeepSeek, OpenRouter, local Ollama, or any OpenAI-compatible endpoint
- **Model capability routing** — automatic per-model payload construction; supports normal models, reasoning/thinking models (DeepSeek-Reasoner, Claude think, etc.), and parameter-restricted models (GPT-5.4 family) without manual configuration
- **Transport resilience** — automatic retry with backoff for transient SSL/connection errors from upstream providers
...
│   ├── llm/               Provider-agnostic LLM facade, capability routing, response normalization
│   │   └── providers/     Abstract base + OpenAI-compatible transport (httpx, retry)
...
- Provider abstraction — `BaseProvider` / `OpenAICompatibleProvider` separate HTTP transport from response normalization; a `ModelCapabilities` layer auto-detects per-model parameter support (temperature, token limits, response format) so reasoning models, GPT-5.4 variants, and standard models all work without manual flags
...
simcoach uses a capability-routing layer that automatically adjusts API parameters per model. No manual configuration is needed — just set your model name in `config.yaml` or the GUI.

| Model family | Status | Notes |
|---|---|---|
| GPT-4o / GPT-4o-mini | Stable | Standard parameters; JSON mode supported |
| GPT-5.4 / GPT-5.4-mini / GPT-5.4-nano | Stable | Temperature and max_tokens suppressed automatically |
| GPT-5.4-high / GPT-5.4-pro | Stable | Token budget floor raised to 16 384 to prevent reasoning exhaustion |
| DeepSeek-chat | Stable | Standard parameters |
| DeepSeek-reasoner | Stable | Reasoning mode; `max_completion_tokens` used, temperature stripped |
| Claude (via OpenRouter / proxy) | Stable | Content-block extraction handles Anthropic-style responses |
| Claude think models (via proxy) | Experimental | Provider-dependent; `thinking` field extraction supported but upstream reliability varies |
| Local Ollama | Stable | Any OpenAI-compatible model |

**Heavy reasoning models** (GPT-5.4-high, GPT-5.4-pro): if the model exhausts its entire completion budget on internal reasoning without producing a final answer, simcoach reports a clear error with guidance to increase `max_tokens` or use a lighter model.

**Transport retry**: transient network errors (SSL disconnects, connection resets) are retried up to 2 times with exponential backoff. HTTP 4xx/5xx errors are not retried.
...
- **提供商无关** — 兼容 OpenAI、DeepSeek、OpenRouter、本地 Ollama 及任何 OpenAI 兼容接口
- **模型能力路由** — 按模型自动构建请求参数；支持普通模型、推理/思考类模型（DeepSeek-Reasoner、Claude think 等）和参数受限模型（GPT-5.4 系列），无需手动配置
- **传输容错** — 对上游提供商的 SSL/连接临时错误自动重试并指数退避
...
- 提供商抽象 — `BaseProvider` / `OpenAICompatibleProvider` 将 HTTP 传输与响应归一化分离；`ModelCapabilities` 层自动检测每个模型支持的参数（温度、token 上限、响应格式），推理模型、GPT-5.4 系列及标准模型均无需手动配置
...
simcoach 内置能力路由层，可按模型自动调整 API 请求参数。无需手动配置 —— 在 `config.yaml` 或 GUI 中设置模型名称即可。

| 模型系列 | 状态 | 说明 |
|---|---|---|
| GPT-4o / GPT-4o-mini | 稳定 | 标准参数；支持 JSON 模式 |
| GPT-5.4 / GPT-5.4-mini / GPT-5.4-nano | 稳定 | 自动抑制 temperature 和 max_tokens |
| GPT-5.4-high / GPT-5.4-pro | 稳定 | token 预算下限提升至 16 384，防止推理耗尽 |
| DeepSeek-chat | 稳定 | 标准参数 |
| DeepSeek-reasoner | 稳定 | 推理模式；使用 `max_completion_tokens`，去除 temperature |
| Claude（通过 OpenRouter / 代理） | 稳定 | 内容块提取兼容 Anthropic 风格响应 |
| Claude think 模型（通过代理） | 实验性 | 取决于代理稳定性；支持 `thinking` 字段提取，但上游可靠性不一 |
| 本地 Ollama | 稳定 | 任何 OpenAI 兼容模型 |

**重度推理模型**（GPT-5.4-high、GPT-5.4-pro）：若模型将全部 completion 预算用于内部推理而未产生最终回答，simcoach 会给出明确错误提示，建议增加 `max_tokens` 或换用更轻量的模型。

**传输重试**：临时性网络错误（SSL 断连、连接重置）最多重试 2 次并指数退避。HTTP 4xx/5xx 错误不会重试。
